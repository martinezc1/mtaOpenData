test_that("mta_any_dataset returns a tibble, respects limits, and supports postprocess toggles", {
  skip_if_not_installed("vcr")
  skip_if_not_installed("curl")

  endpoint <- "https://data.ny.gov/resource/2ucp-7wg5.json"

  vcr::use_cassette("mta_any_dataset_robust", {
    base <- mta_any_dataset(endpoint, limit = 2)
    ...
    raw_names <- mta_any_dataset(endpoint, limit = 2, clean_names = FALSE, coerce_types = FALSE)
