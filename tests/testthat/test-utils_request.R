test_that(".mta_coerce_types handles empty or non-character columns", {
  coerce <- mtaOpenData:::.mta_coerce_types
...
test_that(".mta_add_filters handles quotes and empty strings", {
  add_f <- mtaOpenData:::.mta_add_filters

  # Test the single quote escape logic
  q <- add_f(list(), list(name = "O'Reilly"))
  expect_match(q[["$where"]], "''")

  # Trigger the .mta_abort for empty/whitespace values
  expect_error(add_f(list(), list(agency = " ")), "cannot be empty")
})

test_that(".mta_add_where appends correctly", {
  add_w <- mtaOpenData:::.mta_add_where
...
test_that(".mta_build_date_where handles partial ranges", {
  builder <- mtaOpenData:::.mta_build_date_where
...
test_that(".mta_get_json handles HTTP errors", {
  ...
  expect_error(
    mtaOpenData:::.mta_get_json("https://httpbin.org/status/404", list()),
    "HTTP status 404"
  )
})

test_that(".mta_build_date_where throws errors on conflicting args", {
  builder <- mtaOpenData:::.mta_build_date_where
