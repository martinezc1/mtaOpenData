# Internal helpers for MTA (Mass Transit Association) Open Data requests ####
# - .mta_endpoint(): constructs the Socrata endpoint URL from a dataset id
# - .mta_add_filters(): adds equality filters (and IN() for multi-values) as a Socrata $where clause
# - .mta_add_where(): appends a raw SoQL WHERE expression (for ranges, >=, <, etc.)
# - .mta_validate_date_yyyy_mm_dd(): validates Date or "YYYY-MM-DD" inputs
# - .mta_build_date_where(): constructs server-side date filters using a dataset's default_date_field
# - .mta_get_json(): executes the request and returns parsed JSON (flattened)
# - .mta_dataset_request(): common pattern for dataset wrapper functions + input validation (r07/r08)

.mta_abort <- function(msg) {
  stop(msg, call. = FALSE)
}

# light, safe type coercion (numeric/logical + ISO datetime when obvious)
.mta_coerce_types <- function(df) {
...
.mta_postprocess <- function(df, clean_names = TRUE, coerce_types = TRUE) {
  ...
  if (isTRUE(coerce_types)) {
    df <- .mta_coerce_types(df)
  }

  tibble::as_tibble(df, .name_repair = "minimal")
}

.mta_validate_limit <- function(limit) {
  if (length(limit) != 1 || is.na(limit)) .mta_abort("`limit` must be a single, non-missing numeric value.")
  if (!is.numeric(limit)) .mta_abort("`limit` must be a single numeric value.")
  if (limit < 0) .mta_abort("`limit` must be between 0 and Inf.")
  if (limit != floor(limit)) .mta_abort("`limit` must be an integer (whole number).")
  as.integer(limit)
}

.mta_validate_timeout <- function(timeout_sec) {
  if (length(timeout_sec) != 1 || is.na(timeout_sec)) {
    .mta_abort("`timeout_sec` must be a single, non-missing numeric value.")
  }
  if (!is.numeric(timeout_sec)) {
    .mta_abort("`timeout_sec` must be a single numeric value.")
  }
  if (timeout_sec <= 0) {
    .mta_abort("`timeout_sec` must be > 0.")
  }
  timeout_sec
}

.mta_validate_filters <- function(filters) {
  if (is.null(filters) || length(filters) == 0) return(list())

  if (!is.list(filters)) {
    .mta_abort("`filters` must be a named list.")
  }
  if (is.null(names(filters)) || any(!nzchar(names(filters)))) {
    .mta_abort("`filters` must be a *named* list (all elements must have names).")
  }
  if (any(vapply(filters, function(x) length(x) == 0, logical(1)))) {
    .mta_abort("`filters` values cannot be empty.")
  }
  if (anyNA(unlist(filters))) {
    .mta_abort("`filters` cannot contain NA values.")
  }

  filters
}

.mta_endpoint <- function(dataset_id) {
  stopifnot(
...
.mta_add_filters <- function(query, filters) {
  ...
  make_clause <- function(field, value) {
    ...
    if (is_text && any(!nzchar(value))) {
      .mta_abort("`filters` character values cannot be empty or whitespace-only.")
    }
...
.mta_add_where <- function(query, where) {
  if (is.null(where) || length(where) == 0) return(query)

  if (!is.character(where) || length(where) != 1 || is.na(where)) {
    .mta_abort("`where` must be a single non-missing character string.")
  }
...
.mta_get_json <- function(endpoint, query, timeout_sec = 30) {
  ...
  resp <- tryCatch(
    ...
    error = function(e) {
      stop(
        paste0(
          "MTA Open Data request failed (network unavailable or API slow).\n",
  ...
  if (httr::http_error(resp)) {
    ...
    stop(
      paste0(
        "MTA Open Data request failed with HTTP status ", status, ".\n",
...
.mta_dataset_request <- function(dataset_id,
                                 limit = 10000,
                                 filters = list(),
                                 order = NULL,
                                 where = NULL,
  ...
  filters <- .mta_validate_filters(filters)

  endpoint <- .mta_endpoint(dataset_id)

  query_list <- list("$limit" = limit)

  if (!is.null(order)) {
    if (!is.character(order) || length(order) != 1 || is.na(order) || !nzchar(order)) {
      .mta_abort("`order` must be a single non-empty character string.")
    }
    query_list[["$order"]] <- order
  }

  query_list <- .mta_add_filters(query_list, filters)
  query_list <- .mta_add_where(query_list, where)

  data <- .mta_get_json(endpoint, query_list, timeout_sec = timeout_sec)
...
.mta_validate_date_yyyy_mm_dd <- function(x, arg) {
  if (is.null(x)) return(NULL)
  if (inherits(x, "Date")) return(format(x, "%Y-%m-%d"))

  if (!is.character(x) || length(x) != 1 || is.na(x) || !nzchar(x)) {
    .mta_abort(paste0("`", arg, "` must be a Date or a non-missing 'YYYY-MM-DD' string."))
  }
  if (!grepl("^\\d{4}-\\d{2}-\\d{2}$", x)) {
    .mta_abort(paste0("`", arg, "` must be in 'YYYY-MM-DD' format."))
  }
  x
}

# Build a SoQL WHERE clause for date filtering using the dataset's date field
.mta_build_date_where <- function(date_field, date = NULL, from = NULL, to = NULL) {
  if (is.null(date_field) || is.na(date_field) || !nzchar(date_field)) return(NULL)

  date <- .mta_validate_date_yyyy_mm_dd(date, "date")
  from <- .mta_validate_date_yyyy_mm_dd(from, "from")
  to   <- .mta_validate_date_yyyy_mm_dd(to, "to")

  if (!is.null(date) && (!is.null(from) || !is.null(to))) {
    .mta_abort("Use either `date` OR `from`/`to`, not both.")
  }
